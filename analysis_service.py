...
def run_analysis_task(task_id: str, excel_path: str, output_dir: str = "output", model_id: str = ""):
    ...
    try:
        ...
        # 生成 Top10 摘要数据
        top10_data = generate_top10_summary(result_df)
        
        # 保存 Top10 摘要到 JSON 文件
        top10_json_path = result_output_path.parent / f"{result_output_path.stem}_top10.json"
        ...
        result_dict = {
            ...
            "top10_file": str(top10_json_path.absolute()),
        ...
        task_manager.add_log(task_id, f"Top10 摘要已保存到: {top10_json_path.absolute()}", "INFO")
...
def generate_top10_summary(result_df: pd.DataFrame) -> List[Dict]:
    """
    生成 Top10 摘要数据（只包含前端展示所需的字段）
    
    Args:
        result_df: 分析结果 DataFrame（已按综合评分排序）
    ...
    # 确保按综合评分排序
    if '综合评分' in result_df.columns:
        sorted_df = result_df.sort_values('综合评分', ascending=False)
    else:
        sorted_df = result_df
    ...
    for _, row in top10_df.iterrows():
        summary = {
            ...
            '工作地点': row.get('工作地点', ''),
            '薪资范围': row.get('薪资范围', ''),
            '综合评分': float(row.get('综合评分', 0)) if pd.notna(row.get('综合评分')) else 0,
            '细分评分': float(row.get('细分评分', 0)) if pd.notna(row.get('细分评分')) else 0,
            '信息完整度': float(row.get('信息完整度', 0)) if pd.notna(row.get('信息完整度')) else 0,
            '可执行性': float(row.get('可执行性', 0)) if pd.notna(row.get('可执行性')) else 0,
            '岗位链接': row.get('岗位链接', ''),
            '工作内容': row.get('工作内容', ''),
            '必备技能': row.get('必备技能', ''),
            '评分理由': row.get('评分理由', '')
        }
        top10_data.append(summary)
